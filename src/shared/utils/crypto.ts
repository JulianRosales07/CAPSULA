/**
 * Utilidades criptográficas en Frontend: Cifrado César + Cifrado Asimétrico RSA (2048 bits)
 * Usado para descifrar respuestas y cifrar peticiones hacia el backend.
 */
...
export function caesarDecrypt(base64: string, shift: number): string {
  ...
  for (let i = 0; i < binary.length; i++) {
    bytes[i] = (binary.charCodeAt(i) - shift + 256) % 256
  }
